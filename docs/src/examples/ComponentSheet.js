...
export default class ComponentSheet extends React.Component {
  ...
  generateGrid () {
    ...
    rows = rows.concat(_.range(1, this.state.items + 1).map(id => [{readOnly: true, value: `Item ${id}`},{value: '', component: component(id)}]))

    // rows = rows.concat([[{readOnly: true, value: 'Total'}, {readOnly: true, value: `${total.toFixed(2)} $`}]])//所有的美元数
    console.log(rows)
    return rows
  }
